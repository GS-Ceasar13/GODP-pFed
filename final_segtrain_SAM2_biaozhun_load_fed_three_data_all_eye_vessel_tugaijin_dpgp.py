...
CONFIG = {
    ...
    # 客户端数据路径
    'client1_data_root': '/data1/gs/RP-SAM2-main/Retina',
    'client2_data_root': '/data1/gs/RP-SAM2-main/DRIVE',
    'client3_data_root': '/data1/gs/RP-SAM2-main/FIVES',
    'save_dir': './federated_checkpoints_sam2_all_eye_vessel_tugaijin_dpgp',
    ...
    # 客户端1（Retina）设置
    'client1_train_ratio': 0.8,
    'client1_val_ratio': 0.2,
    
    # 【修改】结果保存设置 - 只保留最佳
    'save_predictions': True,
    'pred_samples_per_round': 3,
    'save_best_predictions': True,
    'eval_freq': 1,                      # 评估频率保持5轮
...
CLIENT1_CLASS_NAMES = [
    "Background",  # 类别0：背景
    "Retina"       # 类别1：视网膜病变/血管等
]

CLIENT2_CLASS_NAMES = [
    "Background",  # 类别0：背景
    "Vessel"       # 类别1：血管（DRIVE数据集）
]

CLIENT3_CLASS_NAMES = [
    "Background",  # 类别0：背景
    "Foreground"   # 类别1：前景（FIVES数据集）
]
...
# ==================== 数据集类（改进版）====================
class ImprovedRetinaDataset(Dataset):
    """改进的Retina数据集，带数据分布感知的点初始化"""
    def __init__(self, data_root, data_split='train', transform=None, is_training=True, use_subset='train'):
    ...
    def _load_samples(self):
        if self.data_split == 'train':
            img_dir = self.data_root / "train" / "image"
            mask_dir = self.data_root / "train" / "mask"
        else:
            img_dir = self.data_root / "test" / "image"
            mask_dir = self.data_root / "test" / "mask"
        
        if not img_dir.exists() or not mask_dir.exists():
            logger.error(f"图像或掩码目录不存在: {img_dir}, {mask_dir}")
            return
        ...
        if self.data_split == 'train':
            train_samples, val_samples = train_test_split(
                all_samples, test_size=CONFIG['client1_val_ratio'], 
                random_state=CONFIG['random_seed']
            )
            
            if self.use_subset == 'train':
        ...
        logger.info(f"Retina数据集 {self.data_split}-{self.use_subset} 加载了 {len(self.samples)} 个样本")
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            logger.warning(f"加载样本失败 {img_path}: {e}")
    ...
    def _process_mask_adaptive(self, mask_path, original_size):
        ...
        try:
            ...
            mask_img = mask_img.resize((CONFIG['image_size'], CONFIG['image_size']), Image.NEAREST)
            mask_array = np.array(mask_img)
            
            masks = torch.zeros(CONFIG['client1_num_classes'], CONFIG['image_size'], CONFIG['image_size'])
            ...
            logger.warning(f"处理掩码失败 {mask_path}: {e}")
    ...
    def _get_grid_points(self):
        ...
        grid_size = int(np.sqrt(CONFIG['num_points_grid']))
        for i in range(grid_size):
            for j in range(grid_size):
                x = (i + 0.5) * CONFIG['image_size'] / grid_size
                y = (j + 0.5) * CONFIG['image_size'] / grid_size
                points.append([x, y])
                labels.append(0)
        
        while len(points) < CONFIG['num_points_grid']:
            x = random.random() * CONFIG['image_size']
            y = random.random() * CONFIG['image_size']
            points.append([x, y])
            labels.append(0)
        
        points = torch.tensor(points[:CONFIG['num_points_grid']], dtype=torch.float32)
        labels = torch.tensor(labels[:CONFIG['num_points_grid']], dtype=torch.long)
    ...
    def _get_empty_sample(self, img_path):
        ...
        return {
            ...
            'masks': torch.zeros(CONFIG['client1_num_classes'], CONFIG['image_size'], CONFIG['image_size']),
            'points': torch.zeros(CONFIG['num_points_grid'], 2),
            'labels': torch.zeros(CONFIG['num_points_grid'], dtype=torch.long),
            'image_path': img_path,
            'mask_path': None,
            'distribution_stats': {}
        }
...
# 类似地改进VesselDataset和NewDataset
# ==================== 修复DRIVE数据集处理 ====================
class ImprovedVesselDataset(Dataset):
    """改进的血管分割数据集 - DRIVE"""
    def __init__(self, data_root, data_split='training', transform=None, is_training=True):
        self.data_root = Path(data_root)
        self.data_split = data_split
        self.transform = transform
        self.is_training = is_training
    ...
    def _load_samples(self):
        ...
        if self.data_split == 'training':
            img_dir = self.data_root / "training" / "images"
            mask_dir = self.data_root / "training" / "labels"
        else:
            # DRIVE测试集的正确路径
            img_dir = self.data_root / "test" / "images"
            mask_dir = self.data_root / "test" / "labels"
        
        if not img_dir.exists() or not mask_dir.exists():
            logger.error(f"图像或掩码目录不存在: {img_dir}, {mask_dir}")
            return
        
        image_files = list(img_dir.glob("*.png")) + list(img_dir.glob("*.jpg")) + list(img_dir.glob("*.tif"))
        
        for img_file in image_files:
            # 尝试不同的掩码文件名格式
            mask_file = mask_dir / f"{img_file.stem}.png"
            if not mask_file.exists():
                mask_file = mask_dir / f"{img_file.stem}.gif"
            if not mask_file.exists():
                mask_file = mask_dir / f"{img_file.stem}_manual1.gif"
            
            if mask_file.exists():
                self.samples.append((str(img_file), str(mask_file)))
        
        logger.info(f"DRIVE数据集 {self.data_split} 加载了 {len(self.samples)} 个样本")
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            masks, points, labels, dist_stats = self._process_mask_adaptive(mask_path)
    ...
    def _process_mask_adaptive(self, mask_path):
        """使用自适应点采样处理掩码 - 修复DRIVE血管识别"""
        try:
            mask_img = Image.open(mask_path).convert('L')
            mask_img = mask_img.resize((CONFIG['image_size'], CONFIG['image_size']), Image.NEAREST)
            mask_array = np.array(mask_img)
            ...
            logger.debug(f"DRIVE掩码唯一值: {unique_values}")
            
            masks = torch.zeros(CONFIG['client2_num_classes'], CONFIG['image_size'], CONFIG['image_size'])
            
            # 重要修复：DRIVE数据集中血管可能用1或255表示
            # 使用更智能的二值化策略
            if len(unique_values) <= 2:
                ...
                # 多值图像，使用Otsu阈值或固定低阈值
                # DRIVE通常血管值较低（1）或较高（255）
                if mask_array.max() > 100:
                    # 高值表示血管（如255）
                    threshold = 127
                else:
                    # 低值表示血管（如1）
                    threshold = 0.5
                foreground_mask = (mask_array > threshold).astype(np.float32)
                background_mask = (mask_array <= threshold).astype(np.float32)
            
            # 验证血管区域是否合理（应该占图像的5-15%左右）
            vessel_ratio = foreground_mask.sum() / (mask_array.shape[0] * mask_array.shape[1])
            logger.debug(f"血管区域比例: {vessel_ratio:.3f}")
            
            # 如果血管区域异常（太大或太小），尝试反转
            if vessel_ratio > 0.5:  # 如果"血管"占了超过50%，可能识别反了
                logger.warning(f"检测到血管区域异常大({vessel_ratio:.3f})，尝试反转")
                foreground_mask, background_mask = background_mask, foreground_mask
                vessel_ratio = 1 - vessel_ratio
            ...
            if self.is_training:
                ...
                # 添加血管区域比例到统计
                dist_stats['vessel_ratio'] = vessel_ratio
            else:
                points, labels = self._get_grid_points()
                dist_stats = {'vessel_ratio': vessel_ratio}
...
class ImprovedNewDataset(Dataset):
    """改进的新数据集 - FIVES"""
    def __init__(self, data_root, data_split='train', transform=None, is_training=True):
        self.data_root = Path(data_root)
        self.data_split = data_split
        self.transform = transform
        self.is_training = is_training
    ...
    def _load_samples(self):
        ...
        if self.data_split == 'train':
            # FIVES训练集的正确路径
            img_dir = self.data_root / "train" / "Original"
            mask_dir = self.data_root / "train" / "Ground truth"
        else:
            # FIVES测试集的正确路径
            img_dir = self.data_root / "test" / "Original"
            mask_dir = self.data_root / "test" / "Ground truth"
        ...
        logger.info(f"FIVES数据集 {self.data_split} 加载了 {len(self.samples)} 个样本")
    ...
    def _process_mask_adaptive(self, mask_path):
        ...
        try:
            ...
            masks = torch.zeros(CONFIG['client3_num_classes'], CONFIG['image_size'], CONFIG['image_size'])
...
class ImprovedFederatedServer:
    ...
    def initialize_global_model(self, checkpoint_path):
        ...
        temp_model = ImprovedGraphAwareSAM2Model(
            num_classes=CONFIG['client1_num_classes'],
...
class ImprovedFederatedClient:
    """改进的联邦学习客户端，完全隔离个性化层"""
    def __init__(self, client_id, data_root, num_classes, class_names, dataset_type, device):
    ...
    def _load_data(self):
        ...
        if self.dataset_type == 'retina':
            train_dataset = ImprovedRetinaDataset(
                self.data_root, 'train', train_transform, True, 'train'
            )
            test_dataset = ImprovedRetinaDataset(
                self.data_root, 'test', test_transform, False
            )
        elif self.dataset_type == 'DRIVE':  # 改为DRIVE
            # 使用ImprovedVesselDataset
            train_dataset = ImprovedVesselDataset(
                self.data_root, 'training', train_transform, True
            )
            test_dataset = ImprovedVesselDataset(
                self.data_root, 'test', test_transform, False
            )
        elif self.dataset_type == 'FIVES':  # 改为FIVES
            # 使用ImprovedNewDataset
            train_dataset = ImprovedNewDataset(
                self.data_root, 'train', train_transform, True
            )
            test_dataset = ImprovedNewDataset(
                self.data_root, 'test', test_transform, False
            )
...
# ==================== 数据集检查函数 ====================
def check_retina_dataset(data_root):
    """检查Retina数据集结构"""
    data_root = Path(data_root)
    
    if not data_root.exists():
        logger.error(f"Retina数据根目录不存在: {data_root}")
        return False
    ...
    if not train_mask_dir.exists():
        logger.error(f"训练掩码目录不存在: {train_mask_dir}")
        return False
    ...
    if not test_img_dir.exists():
        logger.error(f"测试图像目录不存在: {test_img_dir}")
        return False
        
    if not test_mask_dir.exists():
        logger.error(f"测试掩码目录不存在: {test_mask_dir}")
        return False
    
    # 统计文件
    train_img_files = list(train_img_dir.glob("*.png"))
    train_mask_files = list(train_mask_dir.glob("*.png"))
    
    train_valid_pairs = 0
    for img_file in train_img_files:
        mask_file = train_mask_dir / f"{img_file.stem}.png"
        if mask_file.exists():
            train_valid_pairs += 1
    
    test_img_files = list(test_img_dir.glob("*.png"))
    test_mask_files = list(test_mask_dir.glob("*.png"))
    
    test_valid_pairs = 0
    for img_file in test_img_files:
        mask_file = test_mask_dir / f"{img_file.stem}.png"
        if mask_file.exists():
            test_valid_pairs += 1
    
    logger.info(f"Retina数据集统计:")
    logger.info(f"   训练集图像: {len(train_img_files)} 个")
    logger.info(f"   训练集掩码: {len(train_mask_files)} 个")
    logger.info(f"   训练集有效对: {train_valid_pairs} 个")
    logger.info(f"   测试集图像: {len(test_img_files)} 个")
    logger.info(f"   测试集掩码: {len(test_mask_files)} 个")
    logger.info(f"   测试集有效对: {test_valid_pairs} 个")
    
    return train_valid_pairs > 0 and test_valid_pairs > 0

def check_drive_dataset(data_root):
    """检查DRIVE数据集结构"""
    data_root = Path(data_root)
    
    if not data_root.exists():
        logger.error(f"DRIVE数据根目录不存在: {data_root}")
        return False
    ...
    if not train_labels_dir.exists():
        logger.error(f"训练标签目录不存在: {train_labels_dir}")
        return False
    
    image_files = list(train_images_dir.glob("*.png")) + list(train_images_dir.glob("*.jpg"))
    label_files = list(train_labels_dir.glob("*.png"))
    
    valid_pairs = 0
    for img_file in image_files:
        label_file = train_labels_dir / f"{img_file.stem}.png"
        if label_file.exists():
            valid_pairs += 1
    
    logger.info(f"DRIVE训练集统计:")
    logger.info(f"   图像文件: {len(image_files)} 个")
    logger.info(f"   标签文件: {len(label_files)} 个") 
    logger.info(f"   有效图像-标签对: {valid_pairs} 个")
    
    return valid_pairs > 0

def check_new_dataset(data_root):
    """检查新数据集结构"""
    data_root = Path(data_root)
    
    if not data_root.exists():
        logger.error(f"新数据集根目录不存在: {data_root}")
        return False
    ...
    train_gt_dir = train_dir / "Ground truth"
    
    if not train_original_dir.exists():
        logger.error(f"训练原图目录不存在: {train_original_dir}")
        return False
        
    if not train_gt_dir.exists():
        logger.error(f"训练标签目录不存在: {train_gt_dir}")
        return False
    
    train_img_files = list(train_original_dir.glob("*.png")) + list(train_original_dir.glob("*.jpg"))
    train_gt_files = list(train_gt_dir.glob("*.png"))
    
    train_valid_pairs = 0
    for img_file in train_img_files:
        gt_file = train_gt_dir / f"{img_file.stem}.png"
        if gt_file.exists():
            train_valid_pairs += 1
    ...
    if test_dir.exists():
        ...
        test_gt_dir = test_dir / "Ground truth"
        
        if test_original_dir.exists() and test_gt_dir.exists():
            test_img_files = list(test_original_dir.glob("*.png")) + list(test_original_dir.glob("*.jpg"))
            for img_file in test_img_files:
                gt_file = test_gt_dir / f"{img_file.stem}.png"
                if gt_file.exists():
                    test_valid_pairs += 1
    
    logger.info(f"新数据集统计:")
    logger.info(f"   训练集图像-标签对: {train_valid_pairs} 个")
    logger.info(f"   测试集图像-标签对: {test_valid_pairs} 个")
    
    return train_valid_pairs > 0
...
def main():
    ...
    # 检查数据集
    if not Path(CONFIG['client1_data_root']).exists():
        print(f"❌ 客户端1数据集目录不存在: {CONFIG['client1_data_root']}")
        return
    
    if not Path(CONFIG['client2_data_root']).exists():
        print(f"❌ 客户端2数据集目录不存在: {CONFIG['client2_data_root']}")
        return
    
    if not Path(CONFIG['client3_data_root']).exists():
        print(f"❌ 客户端3数据集目录不存在: {CONFIG['client3_data_root']}")
        return
    ...
    try:
        # 检查数据集结构
        logger.info("🔍 检查数据集结构...")
        if not check_retina_dataset(CONFIG['client1_data_root']):
            logger.error("客户端1 (Retina) 数据集结构检查失败！")
            return
        
        if not check_drive_dataset(CONFIG['client2_data_root']):
            logger.error("客户端2 (DRIVE) 数据集结构检查失败！")
            return
        
        if not check_new_dataset(CONFIG['client3_data_root']):
            logger.error("客户端3 (新数据集) 数据集结构检查失败！")
            return
        ...
        # 客户端1：Retina
        client1 = ImprovedFederatedClient(
            client_id=1,
            data_root=CONFIG['client1_data_root'],
            num_classes=CONFIG['client1_num_classes'],
            class_names=CLIENT1_CLASS_NAMES,
            dataset_type='retina',
            device=device
        )
        ...
        # 客户端2：DRIVE
        client2 = ImprovedFederatedClient(
            client_id=2,
            data_root=CONFIG['client2_data_root'],
            num_classes=CONFIG['client2_num_classes'],
            class_names=CLIENT2_CLASS_NAMES,
            dataset_type='DRIVE',
            device=device
        )
        ...
        # 客户端3：FIVES
        client3 = ImprovedFederatedClient(
            client_id=3,
            data_root=CONFIG['client3_data_root'],
            num_classes=CONFIG['client3_num_classes'],
            class_names=CLIENT3_CLASS_NAMES,
            dataset_type='FIVES',
            device=device
        )
        ...
        logger.info(f"   客户端1 (Retina) - 最佳Dice: {best_metrics['client1_dice']:.4f}")
        logger.info(f"   客户端2 (DRIVE) - 最佳Dice: {best_metrics['client2_dice']:.4f}")
        logger.info(f"   客户端3 (FIVES) - 最佳Dice: {best_metrics['client3_dice']:.4f}")
